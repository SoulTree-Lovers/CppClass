...
class Clock:
    ...
    def __init__(self, hour, minute, second):
        ...
        #self.hour.set(hour)    #여기서 self.hour는 Counter클래스의 인스턴스이므로 set메소드는 Counter 클래스의 메소드이다.
        #self.minute.set(minute)
        #self.second.set(second)
        #Clock클래스의 set메소드를 작성한 후, 위 세 줄을 아래의 코드로 변환

        self.set(hour, minute, second) # 여기서 self는 Clock의 인스턴스 이므로 self.set은 Clock클래스의 메소드이다!
